  ...
  #    os.makedirs(folder_results)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(num_epochs):
            ...
            precision_list.append(precision.eval(session=sess))
            recall_list.append(recall.eval(session=sess))
            cls_IoU_list.append(cls_IoU.eval(session=sess))
            ...
            df.to_csv(r'/data/semantic3d_data/Open3D-PointNet2-Semantic3D/result/cm.txt', index = False,header = True,sep=' ', float_format='%.3f')
            #print(cls_IoU_list)
