...
def main():
   ...
   # miou, update_op = tf.metrics.mean_iou(
        #labels_sampled, predictions, setting.num_class,weights=labels_weights_sampled)
   # _ = tf.summary.scalar(predictions_tag, miou)
    confusion_matrix = tf.get_default_graph().get_tensor_by_name('metrics/mean_iou/total_confusion_matrix:0')
    
    # Calculate precision and recall matrix
    diag = tf.reshape(tf.linalg.tensor_diag_part(confusion_matrix), [-1, 1])
    precision = diag / (1e-20+ tf.reshape(tf.reduce_sum(confusion_matrix, 1), [-1, 1]))
    recall = diag / (1e-20+tf.reshape(tf.reduce_sum(confusion_matrix, 0), [-1, 1]))
    cls_IoU = diag/(1e-20+tf.reshape(tf.reduce_sum(confusion_matrix, 1), [-1, 1])+tf.reshape(tf.reduce_sum(confusion_matrix, 0), [-1, 1])-diag)
    precision_list=[]
    recall_list=[]
    cls_IoU_list=[]
   ...
   # miou_op = tf.print("Miou:\n", miou,
                     #    output_stream=sys.stdout)
    #L = [ 'Unclassified','natural_ground','low_vegetation','high_vegetation','buildings','road','wire_structure','cars','van','bus','bike','trash_can','street_signs','lamps','remaining_hardscapes','crane','greenhouses','wall','manmade_terrain','manhole_cover','persons','reflexions','barrier','wood_structure','cycling_path']
    #figure = plot_confusion_matrix(recall, labels=L)
    #cm_image = plot_to_image(figure)


    #_ = tf.summary.image("Confusion Matrix", cm_image, collections=['val'])

    # Add precision and recall matrix in Tensorboard
    _ = tf.summary.text('diagonal', tf.dtypes.as_string(diag, precision=4), collections=['val'])
    _ = tf.summary.text('recall_matrix', tf.dtypes.as_string(recall, precision=4), collections=['val'])
    _ = tf.summary.text('precision_matrix', tf.dtypes.as_string(precision, precision=4), collections=['val'])
    _ = tf.summary.text('cls_IoU', tf.dtypes.as_string(cls_IoU, precision=4), collections=['val'])
  ...
  #  folder_results = os.path.join(root_folder, 'results')
  ...
  #    os.makedirs(folder_results)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(num_epochs):
            ...
            precision_list.append(precision.eval(session=sess))# produce OA per class
            recall_list.append(recall.eval(session=sess))  #produce recall class
            cls_IoU_list.append(cls_IoU.eval(session=sess)) #produce IOUs per class
            rec = np.array(recall_list).flatten()  
            preci = np.array(precision_list).flatten() 
            iou = np.array(cls_IoU_list).flatten()
            df = pd.DataFrame(list(zip(preci, rec,iou)),  columns =['precision', 'recall','iou'])
            df.to_csv(r'/data/semantic3d_data/Open3D-PointNet2-Semantic3D/result/cm.txt', index = False,header = True,sep=' ', float_format='%.3f') #save OA recall and IOUs per class for further analysis
            #print(cls_IoU_list)
            #log_string(cls_IoU_list)
