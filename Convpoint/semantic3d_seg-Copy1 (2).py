...
def main():
    ...
    parser.add_argument('--block_size', help='Block size', type=float, default=64)
    ...
    filelist_train=[
       "80101_xyz_intensity_rgb_voxels.npy",
        "80102_xyz_intensity_rgb_voxels.npy",
        "8010_xyz_intensity_rgb_voxels.npy",      
        "8052_xyz_intensity_rgb_voxels.npy",
        "8048_xyz_intensity_rgb_voxels.npy", 
    ]
 #"8050_xyz_intensity_rgb_voxels.npy",
    filelist_test = [
                "8050_xyz_intensity_rgb_voxels.npy", 
        "rescaled_8054_8055_106267_106281_voxels.npy",
        "rescaled_8056_8057_106260_102280_voxels.npy",
        "rescaled_8058_8059_106261_106281_voxels.npy",
        ]
        #filelist_test = [
       # "rescaled_8054_8055_106267_106281_voxels.npy",
       # "rescaled_8056_8057_106260_102280_voxels.npy",
    ...
    if not args.test:
        ...
        for epoch in range(args.epochs):
            ...
            for pts, features, seg in t:
                ...
                f1score = f"{metrics.stats_f1score_per_class(cm)[0]:.5f}"
                
                l1iou = metrics.stats_iou_per_class(cm)[1]
                l1acc = metrics.stats_accuracy_per_class(cm)[1]
                l1f1 = metrics.stats_f1score_per_class(cm)[1]
            ...
            lacc.append(l1acc)
            lf1.append(l1f1)
            ious = np.array(liou).flatten()
            acc = np.array(lacc).flatten()
            f1 = np.array(lf1).flatten()
            df = pd.DataFrame(list(zip(ious,acc,f1)),  columns =['iou','precision','f1score'])
            df.to_csv((root_folder + '/trainingresults.txt'), index = False,header = True,sep=' ', float_format='%.3f')##save IOU OA and f1 score per class for further analysis
            # write the logs
            logs.write(f" {epoch} {oa} {aa} {iou} {f1score} \n")
            logs.flush()
